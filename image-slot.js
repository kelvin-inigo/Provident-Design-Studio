...
(() => {
  ...
  // raw upload.
  //
  // Provident Design Studio: one export-grade copy per image, produced by
  // runtime.js so the canvas and every export read the same bytes and can't
  // disagree. It caps the long side at 3840px (2× the widest canvas, 16:9 at
  // 3840×2160, so no export upscales), picks WebP / JPEG / PNG by what the
  // browser can actually encode and whether the image needs alpha, and only
  // trades quality for size past a byte budget. The block below is the
  // fallback for when runtime.js isn't present.
  async function toDataUrl(file, targetW) {
    if (window.providentEncodeFile) return await window.providentEncodeFile(file);
    const CAP = 2560;
    ...
    try {
      ...
      if (long <= CAP && file.size <= 1500000) return await readRaw();
  ...
  async function toDataUrl_downscaled_unused(file, targetW) {
    const bitmap = await createImageBitmap(file);
    try {
  ...
  const stylesheet =
    ...
    ':host([data-reframe]) .rfc{display:flex}' +
    '.rfc:popover-open{position:fixed;inset:auto;display:flex}' +
    '.rfc-r{display:flex;align-items:center;gap:5px;flex-wrap:wrap}' +
    '.rfc-l{opacity:.7;flex:none}' +
    '.rfc-v{opacity:.7;flex:none;min-width:34px;text-align:right;font-variant-numeric:tabular-nums}' +
    // Was accent-color:#C9A96A — brass, from before the chrome accent went blue, and
    // rendered NATIVELY (thick filled bar, big thumb) unlike every other slider.
    // CSS custom properties DO inherit through a shadow boundary, so var(--ps-accent)
    // resolves from :root here and this stays on the design system for free.
    '.rfc-z{-webkit-appearance:none;appearance:none;flex:1;min-width:80px;height:14px;' +
      'background:transparent;cursor:pointer}' +
    '.rfc-z::-webkit-slider-runnable-track{height:4px;border-radius:999px;' +
      'background:var(--ps-line,rgba(255,255,255,.13))}' +
    // A THUMB IS A MARK. --ps-accent is now #101E39, which is invisible on this scrim, and
    // --ps-link cannot carry it either: this overlay's ground does NOT follow the theme
    // (its ink is #fff and its borders are white alphas), so in light mode --ps-link would
    // be #13459A at 1.5:1 here. White is the ink already beside it, and needs no theme.
    '.rfc-z::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:14px;' +
      'height:14px;margin-top:-5px;border:none;border-radius:50%;' +
      'background:#fff;box-shadow:0 1px 4px rgba(0,0,0,.35)}' +
    '.rfc-z::-moz-range-track{height:4px;border-radius:999px;' +
      'background:var(--ps-line,rgba(255,255,255,.13))}' +
    '.rfc-z::-moz-range-thumb{width:14px;height:14px;border:none;border-radius:50%;' +
      'background:#fff}' +
    '.rfc-sep{width:1px;height:16px;background:rgba(255,255,255,.25);flex:none;margin:0 2px}' +
    '.rfc button{appearance:none;border:1px solid rgba(255,255,255,.22);border-radius:6px;' +
    '  min-width:22px;padding:4px 7px;cursor:pointer;background:rgba(255,255,255,.08);' +
    '  color:#fff;font:11px/1 system-ui,-apple-system,sans-serif}' +
    '.rfc button:hover{background:rgba(255,255,255,.2)}' +
    '.rfc button[data-on]{background:var(--ps-accent,#101E39);' +
      'border-color:rgba(255,255,255,.55);color:#fff}' +
    '.rfc .rfc-done{background:var(--ps-accent-h,#2053D0);' +
      'border-color:transparent;color:#fff;font-weight:500}' +
    '.rfc-h{opacity:.55;flex:1;text-align:right;min-width:0}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    static copySlot(fromId, toId) {
      if (!loaded || !fromId || !toId || fromId === toId) return false;
      const v = getSlot(fromId);
      if (!v) return false;
    ...
    static moveSlot(fromId, toId) {
      ...
      setSlot(toId, v ? Object.assign({}, v) : null);
      if (fromId && fromId !== toId) setSlot(fromId, null);
      return true;
    }
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '<div class="rfc" popover="manual" data-dc-edit-transparent>' +
        '  <div class="rfc-r"><span class="rfc-l">Zoom</span>' +
        '    <input class="rfc-z" type="range" min="100" max="400" step="1" value="100">' +
        '    <span class="rfc-v">100%</span></div>' +
        '  <div class="rfc-r"><button data-rf="fill">Fill</button><button data-rf="fit">Fit</button>' +
        '    <span class="rfc-sep"></span>' +
        '    <button data-rf="a-tl" title="Anchor top left">\u2196</button>' +
        '    <button data-rf="a-t"  title="Anchor top">\u2191</button>' +
        '    <button data-rf="a-tr" title="Anchor top right">\u2197</button>' +
        '    <button data-rf="a-l"  title="Anchor left">\u2190</button>' +
        '    <button data-rf="a-c"  title="Centre">\u25cf</button>' +
        '    <button data-rf="a-r"  title="Anchor right">\u2192</button>' +
        '    <button data-rf="a-bl" title="Anchor bottom left">\u2199</button>' +
        '    <button data-rf="a-b"  title="Anchor bottom">\u2193</button>' +
      ...
      this._rfc = root.querySelector('.rfc');
      this._rfz = root.querySelector('.rfc-z');
      this._rfv = root.querySelector('.rfc-v');
      // Never let overlay interaction reach the canvas underneath — a click here
      // would otherwise select the design component or clear the selection.
      ['pointerdown', 'mousedown', 'click', 'dblclick', 'wheel'].forEach((ev) => {
        this._rfc.addEventListener(ev, (e) => e.stopPropagation());
      });
      this._rfz.addEventListener('input', (e) => {
        const next = clampS((+e.target.value || 100) / 100);
        const prev = this._view.s || 1;
        if (next === prev) return;
        // Zoom about the frame centre, same maths as the wheel handler with the
        // cursor at 0,0: offset' = 0*(1-k) + offset*k.
        const k = next / prev;
        this._view.s = next;
        this._view.x *= k;
        this._view.y *= k;
      ...
      this._rfc.addEventListener('click', (e) => {
        const b = e.target.closest && e.target.closest('[data-rf]');
        if (!b) return;
        const act = b.getAttribute('data-rf');
        if (act === 'done') { this._exitReframe(true); return; }
        if (act === 'reset') {
          this._view = { s: 1, x: 0, y: 0, f: this._view.f };
        } else if (act === 'fill' || act === 'fit') {
          // Changing the baseline invalidates the old pan/zoom.
          this._view = { s: 1, x: 0, y: 0, f: act === 'fit' ? 'contain' : 'cover' };
        } else if (act.indexOf('a-') === 0) {
          // Overshoot and let _clampView pull it back to the exact edge.
          const a = act.slice(2), BIG = 1e4;
          this._view.x = a.indexOf('l') >= 0 ? BIG : (a.indexOf('r') >= 0 ? -BIG : 0);
          this._view.y = a.indexOf('t') >= 0 ? BIG : (a.indexOf('b') >= 0 ? -BIG : 0);
        }
        this._clampView();
        this._applyView();
        this._syncRfc();
      ...
      this._spill.addEventListener('pointerdown', (e) => {
        ...
        if (corner) {
          ...
          const contain = this._fitMode() === 'contain';
    ...
    _enterReframe() {
      ...
      try { this._rfc.showPopover(); } catch {}
      this._syncRfc();
    ...
    _exitReframe(commit) {
      ...
      this._spill.style.zoom = this._ctl.style.zoom = this._rfc.style.zoom = '';
      this.removeAttribute('data-panning');
      ...
      try { this._rfc.hidePopover(); } catch {}
      this._ctl.style.left = ''; this._ctl.style.top = '';
      this._rfc.style.left = ''; this._rfc.style.top = ''; this._rfc.style.width = '';
    ...
    async _ingest(file) {
      this._setError(null);
      // Type alone is not enough: an .svg dragged out of Finder, Figma or another
      // browser tab often arrives with an EMPTY file.type, and matching on the
      // MIME list alone rejected it while the placeholder promised "Drop SVG".
      // Fall back to the extension whenever the type is missing or unrecognised.
      const byExt = /\.(svg|png|jpe?g|webp|avif)$/i.test(file && file.name || '');
      if (!file || (ACCEPT.indexOf(file.type) < 0 && !byExt)) {
        this._setError('Drop an SVG, PNG, JPEG, WebP or AVIF file.');
        return;
      }
    ...
    // pan toward zero.
    // Baseline framing: the overlay's persisted choice wins over the attribute.
    _fitMode() {
      var f = this._view && this._view.f ? this._view.f : (this.getAttribute('fit') || 'cover');
    ...
    _syncRfc() {
      if (!this._rfz) return;
      const pct = Math.round((this._view.s || 1) * 100);
      if (+this._rfz.value !== pct) this._rfz.value = String(pct);
      this._rfz.max = String(Math.round(S_MAX * 100));
      if (this._rfv) this._rfv.textContent = pct + '%';
      const fit = (this._view.f ? this._view.f : (this.getAttribute('fit') || 'cover')).toLowerCase() === 'contain';
      const mark = (sel, on) => {
        const b = this._rfc && this._rfc.querySelector(sel);
        if (b) b.toggleAttribute('data-on', on);
      };
      mark('[data-rf="fit"]', fit);
      mark('[data-rf="fill"]', !fit);
    }
    ...
    // Cumulative CSS `zoom` on the ancestor chain, shadow hosts included.
    //
    // The top layer escapes ancestor TRANSFORMS, which is why the popover coordinates
    // below are written in viewport px. It does NOT escape `zoom`: zoom is a used-value
    // multiplier carried down the box tree, so a popover inside a zoomed subtree still
    // gets scaled by it. A host that lays its canvas out at true pixel dimensions and
    // shrinks it with `zoom` therefore rendered the reframe ghost at zoom x its intended
    // size and at zoom x its intended position — the drag surface sat away from the image
    // it was supposed to cover. Dividing the viewport-px values by this factor puts them
    // back. Computed `zoom` reports each element's OWN value (descendants read 1), so the
    // product up the chain is the real factor; with no zoom anywhere this returns 1 and
    ...
    _applyView() {
      ...
      if (this.hasAttribute('data-reframe')) {
        ...
        const unzoom = z === 1 ? '' : String(1 / z);
        this._spill.style.zoom = unzoom;
        this._ctl.style.zoom = unzoom;
        this._rfc.style.zoom = unzoom;
        let onTop = false;
        ...
        let rfcTop = false;
        try { rfcTop = this._rfc.matches(':popover-open'); } catch {}
        if (rfcTop) {
    ...
    _commitView() {
      ...
      if (this._view.f) v.f = this._view.f;
